...
v = [ -5.0000 - 5.00000i, -5.0000 + 5.00000i,-1.00000 - 1.00000i,-1.00000 + 1.00000i];
...
%CONTROLE LQR
Q = [50,0,0,0;0,50,0,0;0,0,50,0;0,0,0,5];
P = [0.01];
klqr = lqr(A,B,Q,P);
X = [zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n)];
X(1,1) = 0;
...
vo = [-7.00000 - 5.00000i, -7.00000 + 5.00000i,-8.00000 - 1.00000i,-8.00000 + 1.00000i];
...
Qo = [50,0,0,0;0,50,0,0;0,0,50,0;0,0,0,5];
Po = [0.01];
...
Git = 351+16.5/s % Método por controle ótimo ITAE
Glr = 48.5 + 23.4/s+27*s %Método lor lugar das raízes

% figure(23)
% nyquist(G2*Gap)
% title("Nyquist alocação de polos")
% figure(24)
% nyquist(G2*Git)
% title("Nyquist ITAE")
% figure(25)
% nyquist(G2*Glr)
% title("Nyquist Lugar das raízes")

%MARGENS DE ESTABILIDADE
figure(26)
hold on
GHap=G2*Gap/(1+G2*Gap)
GHit=G2*Git/(1+G2*Git)
GHlr=G2*Glr/(1+G2*Glr)
% bode(G2*Gap/(1+G2*Gap))
% bode(G2*Git/(1+G2*Git))
% bode(G2*Glr/(1+G2*Glr))
margin(GHlr)
legend("Alocação de polos","ITAE","Luga das raízes")
hold off
grid on
...
% hold on
% step(G2/(1+G2))
% step(G2*Gap/(1+G2*Gap),50)
% step(G2*Git/(1+G2*Git),50)
% step(G2*Glr/(1+G2*Glr),50)
% hold off
% grid on
% title("Comparação sistemas de controle")
% xlabel("Tempo")
% ylabel("Velocidade do robô esférico [m/s]")
% legend("None","AP","ITAE","LR")

%ITAE+PRE COMPENSADOR
Gpc=4.43+8.05/s
Tpc = (G2*Gpc*Git/(1+G2*Gpc*Git))
Tit = (G2*Git/(1+G2*Git))
figure(28)
hold on
step(Tit,8)
...
hold off
grid on
title("Resposta em degrau ITAE+pre-compensador")
xlabel("Tempo")
ylabel("Velocidade do robô esférico [m/s]")
legend("ITAE","ITAE+PC")
...
%Função de transferência que relaciona a posição do robô esférico com a
%sua velocidade

G3 = (G1/G2);

%Respostas em malha fechada para a posição do robô esférico
Tal1 = feedback(Gap*G2,1);
Titae1 = feedback(Git*G2,1);
Tlr1 = feedback(Glr*G2,1);

% figure (29)
% hold on
% step(G1/(1+G1),t); %sem controlador
% step(Tal1*G3,t); %por Alocação de Polos
% step(Titae1*G3,t); %por ITAE
% step(Tlr1*G3,t); %por Lugar das Raízes
% title('Desempenho dos controladores para a posição do robô esférico')
% xlabel('Tempo [s]')
% ylabel('Posição do robô esférico [m]')
% legend('Sem controlador','Alocação','ITAE','LR')
% grid on
% hold off

%COMPARAÇÃO CONTROLE MODERNO X CLÁSSICO
Qo = [50,0,0,0;0,50,0,0;0,0,50,0;0,0,0,5];
Po = [0.01];
kolqr = lqr(transpose(A),transpose(C),Qo,Po)
dt = 0.01;
...
% xlabel("Tempo")
% ylabel("Velocidade do robô esférico [m/s]")
% legend("LQO","PID ITAE + PC")
% title("Comparação moderno x clássico")
